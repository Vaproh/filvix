...
class MusicHandler(commands.Cog):
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload) -> None:
        ...
        source = track.source
        if source == "spotify":
            embed_play.add_field(name="Track Source", value=f"<a:spotify:1226989191150309536> {track.source}")
        elif source == "youtube":
            embed_play.add_field(name="Track Source", value=f"<:Youtube_music:1226989661797486634>  {track.source}")
        else:
