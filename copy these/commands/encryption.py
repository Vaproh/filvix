...
class Encryption(commands.Cog):
    ...
    async def password(self, ctx):
        ...
        if hasattr(ctx, "guild") and ctx.guild is not None:
            await ctx.send(
                f"Sending you a private message with your random generated password <:ios_lol:1125062201938563133>**{ctx.author.name}**"
            )
        await ctx.author.send(
            f"<a:Gifttt:1131840234213101691> **Here is your password:**\n{secrets.token_urlsafe(18)}"
        )
