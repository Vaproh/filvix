...
class PaginationViewWallah:
  ...
  async def start(self, ctx, disxd=False):
    ...
    menu.add_button(bax)
    bax2 = ViewButton(style=discord.ButtonStyle.danger, label=None,emoji='<:zzcross64:1103568431795675187>', custom_id=ViewButton.ID_END_SESSION)
...
class list(commands.Cog):
    ...
    async def seggs(self, ctx):
      ...
      await boost_lis(ctx=ctx, listxd=l, color=0x00FFED, title=f"List of Boosters in {ctx.guild.name} - {int(len(l))}")
    ...
    async def seggss(self, ctx):
      ...
      for i in range(len(ctx.guild.members)):
        sure = ok.get(str(i))
        l.append(ctx.guild.get_member(int(sure)))
      await working_lister(ctx=ctx, listxd=l, color=0x00FFED, title=f"List of Join Position in {ctx.guild.name} - {int(len(l))}")
    ...
    async def seggsss(self, ctx):
      ...
      await working_lister(ctx=ctx, listxd=l, color=0x00FFED, title=f"List of users without any role - {int(len(l))}")
    ...
    async def seggssss(self, ctx):
     ...
     l = []
     await working_listerr(ctx=ctx, listxd=ctx.guild.emojis, color=0x00FFED, title=f"List of Emojis in {ctx.guild.name} -  {a}")
    ...
    async def bots(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"Bots in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
    ...
    async def admins(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"Admins in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
    ...
    async def mods(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"Mods in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
    ...
    async def early(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"Early Id's in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
    ...
    async def activedev(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"Active Developer's in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
    ...
    async def botdev(self, ctx):
      ...
      await working_lister(listxd=loda, color=0x00FFED, title=f"List of developer in {ctx.guild.name} - {int(len(loda))}", ctx=ctx)
   
    @jija.command(name="inrole", aliases=["inside-role"],description="See a list of members that are in the seperate role")
    @blacklist_check()
    @ignore_check()
    async def list_inrole(self, ctx, role: discord.Role):
      l = role.members
    
      await working_lister(ctx=ctx, listxd=l, color=0x00FFED, title=f"List of members in {role.name} - {int(len(l))}")
    ...
    async def list_bans(self, ctx):
      ...
      await lister_bn(ctx=ctx, listxd=ok, color=0x00FFED, title=f"List of Banned members in {ctx.guild.name} - {len(ok)}")
           
    
    @jija.command(name="roles", aliases=["role"],description="See a list of roles in the server")
    @blacklist_check()
    @ignore_check()
    async def list_roles(self, ctx):
      l = [r for r in ctx.guild.roles if not r.id == ctx.guild.default_role.id]
      l.reverse()
      await rolis(ctx=ctx, listxd=l, color=0x00FFED, title=f"List of Roles in {ctx.guild.name} - {int(len(l))}")


    
    @jija.command(name="invc",aliases=["vc","in-vc"],description="See a list of members in a vc")
    @blacklist_check()
    @ignore_check()
    async def list_invc(self, ctx):
      ss = ctx.message.author.voice.channel.name
      if not ctx.message.author.voice:
         await ctx.send(f"<:crosss:1212440602659262505> | You are not connected to any voice channels")
      else:
        loda = []
      ...
      if loda == []:
        ...
        await working_lister(ctx=ctx, listxd=loda, color=0x00FFED, title=f"List Of Members In {ss} - {int(len(loda))}")
