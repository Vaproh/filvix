...
class Verification(commands.Cog):
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<:Black_2:1139197762303447172>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In *{ctx.guild.name}* Press the <:Black_2:1139197762303447172> Button Below", color=0x00FFCA)
