...
class Info(commands.Cog):
  ...
  async def serverinfo(self, ctx: commands.Context):
        ...
        if guild.system_channel_flags.join_notifications:
          haa = "**System Welcome Messages**: <:IconTick:1213170250267492383>"
        else:
          haa = "**System Welcome Messages**: <:crosss:1212440602659262505>"
        afkk = ctx.guild.afk_channel
        if ctx.guild.system_channel_flags.premium_subscriptions:
          skib = "**System Boost Messages**: <:IconTick:1213170250267492383>"
        else:
          skib = "**System Boost Messages**: <:crosss:1212440602659262505>"  
        ...
        if ctx.guild.mfa_level == discord.MFALevel.disabled:
          mfa = "Enabled <:IconTick:1213170250267492383>"
        else:
          mfa = "Disabled <:crosss:1212440602659262505>"
        embed.add_field(
            name="**__Extras__**",
            value=f"""
...
**2FA Requirement:** {mfa}
**Boost Bar Enabled:** {'<:crosss:1212440602659262505>' if not ctx.guild.premium_progress_bar_enabled else '<:IconTick:1213170250267492383>'}
            """,
            inline=False
        )
        embed.add_field(
...
{guild.description}
        ...
        if guild.features:
            ...
            embed.add_field(
                name="**__Features__**",
                value='\n'.join([f"<:IconTick:1213170250267492383>: {feature}" for feature in dk]),
                inline=False
            )
...
Channels: <:hashtag:1216384589799358574> {len(guild.text_channels)} | <:Icon_Mic:1212418073521561700> {len(guild.voice_channels)} | <:Room_icon_Stage:1216388523179900938> {len(guild.stage_channels)} 
{bye}
            """,
            inline=False
        )
...
Total Emoji: {len(guild.emojis)}/{uf}
        ...
        embed.add_field(
            ...
            value=f"""
Level: {guild.premium_tier} <:icons_boost:1216388715396730962> {guild.premium_subscription_count} boosts 
{byecom}
            """,
            inline=False
        )
