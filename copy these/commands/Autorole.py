...
class autorole(commands.Cog):
  ...
  async def autorole(self, ctx):
      ...
      embed.set_footer(text=f"Heavens • Page 1/1",icon_url=self.bot.user.avatar)
  ...
  async def resetall(self, ctx):
      ...
      embed.set_footer(text="Heavens • Page 1/1", icon_url=self.bot.user.avatar)
      await ctx.send(embed=embed)
  ...
  async def humans(self, ctx):
      ...
      embed.set_footer(text=f"Heavens • Page 1/1",icon_url=client.user.avatar)
      await ctx.send(embed=embed)
  ...
  async def auto_rolshow(self, ctx):
    ...
    if human_autoroles == [] and bot_at == []:
      ...
      if bot_at == []:
        ...
        for item in bot_at:
          bye2.append(f"<@&{item}>")
      embed = discord.Embed(title=f"**{ctx.guild.name} Autorole Settings <:nexus_roles:1138644802112602263>**", color=0x00FFCA)
