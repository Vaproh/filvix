...
class PaginationViewWallah:
  ...
  async def start(self, ctx, disxd=False):
    ...
    for xem in self.elist:
      menu.add_page(xem)
    lax = ViewButton(style=discord.ButtonStyle.primary,
                     label='≪',
                     emoji=None,
                     custom_id=ViewButton.ID_GO_TO_FIRST_PAGE)
    menu.add_button(lax)
    bax = ViewButton(style=discord.ButtonStyle.green,
                     label='Back',
                     emoji=None,
    ...
    bax2 = ViewButton(style=discord.ButtonStyle.danger,
                      label=None,
                      emoji='<a:Cross:1115898865460203551>',
                      custom_id=ViewButton.ID_END_SESSION)
    menu.add_button(bax2)
    bax3 = ViewButton(style=discord.ButtonStyle.green,
                      label='Next',
                      emoji=None,
                      custom_id=ViewButton.ID_NEXT_PAGE)
    menu.add_button(bax3)
    sax = ViewButton(style=discord.ButtonStyle.primary,
                     label='≫',
                     emoji=None,
...
class owner1(Cog):
  ...
  def __init__(self, client: Astroz):
    self.client = client
    self.owner_ids = [
      765865384011628574, 1212431696381612132,
      582467613632757783,
      1151589522925174814,
      1212431696381612132,
      1043194242476036107,
  ...
  @_np.command(name="add",
               aliases=["give"],
               description="Add user to no prefix")
  async def np_add(self, ctx: Context, user: discord.User):
    if ctx.message.author.id not in self.owner_ids:
      await ctx.send(
        "<:crosss:1212440602659262505>| this command can only be executed by my developers ."
      )
      return
    ...
    if user.id in np:
      await ctx.send(embed=discord.Embed(
        description=
        f"<:crosss:1212440602659262505> | {user.name} Already have no prefix",
        color=0x0d0d13))
    else:
    ...
    with open('info.json', 'w') as idk:
      ...
      await ctx.send(embed=discord.Embed(
        description=
        f"<:IconTick:1213170250267492383> | Added no prefix to {user.name}",
        color=0x0d0d13))

  @_np.command(name="remove",
               aliases=["rmv"],
               description="Remove user from no prefix")
  async def np_remove(self, ctx: Context, user: discord.User):
    if ctx.message.author.id not in self.owner_ids:
      await ctx.send(
        "<:crosss:1212440602659 262505>| this command can only be executed by my developers ."
      )
      return
    with open('info.json', 'r') as idk:
      data = json.load(idk)
    ...
    if user.id not in np:
      await ctx.send(embed=discord.Embed(
        description=
        f"<:crosss:1212440602659262505> | {user.name} Doesn't have no prefix",
        color=0x0d0d13))
    else:
    ...
    with open('info.json', 'w') as idk:
      ...
      await ctx.send(embed=discord.Embed(
        description=
        f"<:IconTick:1213170250267492383> | Removed no prefix from {user.name}",
        color=0x0d0d13))
  ...
  async def bye_buddy(self, ctx):
    ...
    if file["np"] == []:
      return await ctx.send(
        "<:crosss:1212440602659262505> | No one have no prefix.")
    comrade = []
    for idk in file["np"]:
      member = await self.client.fetch_user(int(idk))
      comrade.append(member)
    await working_lister(title="List Of Users In No Prefix",
                         ctx=ctx,
                         color=0x0d0d13,
                         listxd=comrade)
